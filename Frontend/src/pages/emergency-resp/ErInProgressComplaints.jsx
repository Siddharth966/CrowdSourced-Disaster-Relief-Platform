...
const ErInProgressComplaints = () => {
  ...
  const fetchComplaints = async () => {
    try {
      const inprogress = await getComplaints(["In Progress"]);
      // console.log("pending,pending", pending);
      // Filter non-critical complaints immediately after fetching
      const nonCriticalItems = inprogress.data.filter(
  ...
  const handleAccept = async (complaintId) => {
    try {
      ...
      if (response.status === 200) {
        toast.success("Complaint marked as Done Successfully"); // Show success message
      ...
      console.error("Error Done complaint:", error);
      toast.error("Failed to Done complaint."); // Show error message
    }
  };
  ...
  return (
    <>
      ...
      {isVisible && (
        <Modal
          question="Are you sure you want to end this complaint?"
          onConfirm={() => {
...
export default ErInProgressComplaints;
